...
def make_dir(dataset, model):
    ...
    os.makedirs('./result/%s' % save_dir, exist_ok=True)            # Set save directory
    return save_dir
...
def save_Pred_GT(pred, labels, target_image_path, val_img_ids, num, suffix):
    ...
    image_path=os.path.join(target_image_path.split("result")[0],(val_img_ids[num]))
    print(image_path)
    ...
    # or_label=np.array(label)
    # or_label=cv2.cvtColor(or_label, cv2.COLOR_GRAY2BGR)
    ...
    # img = Image.fromarray(labelsss.reshape(512, 512))
