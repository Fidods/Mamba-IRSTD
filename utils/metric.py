...
class PD_FA():
    ...
    def update(self, preds, labels):
        ...
        for iBin in range(self.bins+1):
            ...
            predits  = np.reshape (predits,  (512,512))
            labelss = np.array((labels).cpu()).astype('int64') # P
            labelss = np.reshape (labelss , (512,512))
...
def cal_tp_pos_fp_neg1(output, target, nclass, score_thresh):
...
class PD_FA1():
    ...
    def update(self, preds, labels):
        ...
        for iBin in range(self.bins+1):
            ...
            labelss = np.array((labels>0).cpu()).astype('int64') # P
...
def cal_tp_pos_fp_neg2(output, target, nclass, score_thresh):
    mini = 1
    maxi = 1 # nclass
    nbins = 1 # nclass

    predict = (output.cpu().detach().numpy() > score_thresh).astype('int64') # P
